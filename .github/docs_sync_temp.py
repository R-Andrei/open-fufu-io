...
def replace_section(text: str, start_heading: str, next_heading: str, replacement: str, label: str) -> str:
    pattern = re.compile(
    ...
    if count != 1:
        raise SystemExit(f"expected one section match for {label}, got {count}")
    return updated


def replace_tail(text: str, start_heading: str, replacement: str, label: str) -> str:
    pattern = re.compile(rf"^{re.escape(start_heading)}\n.*\Z", re.MULTILINE | re.DOTALL)
    updated, count = pattern.subn(replacement.rstrip() + "\n", text, count=1)
    if count != 1:
        raise SystemExit(f"expected one tail-section match for {label}, got {count}")
    return updated
...
design = replace_once(
    ...
    "stale strategic-spawn TODO paragraph",
)
...
integration = replace_once(
    ...
    "integration passive gold classification",
)
integration = replace_once(
    integration,
    ...
    "integration MIRV classification",
)

integration = replace_tail(
    integration,
...
These remaining questions should be resolved by updating these same canonical documents rather than creating additional migration-plan documents.''',
    "integration open-question tail",
)
