...
describe("Factory generated rail-loop planning", () => {
  it("creates no loop for zero eligible stations and includes every eligible station when at most five exist", () => {
    ...
    const result = planFactoryRailLoop(
      factoryPlanningInput(width, height, [
        ...
        Object.freeze({ ...factoryStation("inactive", 4, 4, width), active: false }),
        Object.freeze({ ...factoryStation("unfinished", 8, 4, width), completedLevel: undefined }),
      ]),
    );
  ...
  it("rejects an avoidably self-retracing external loop when a valid simple loop exists", () => {
    const width = 5;
    const height = 5;
    const a = factoryStation("a-station", 2, 1, width);
    const b = factoryStation("b-station", 2, 3, width);
    const result = planFactoryRailLoop(
      factoryPlanningInput(width, height, [a, b], {
        outboundPortCellId: gridCell(0, 2, width),
        inboundPortCellId: gridCell(4, 2, width),
      }),
    );
    ...
    expect(result).not.toBeNull();
    expect(result?.targetStructureIds).toEqual(["a-station", "b-station"]);
    expect(new Set(result?.cells).size).toBe(result?.cells.length);
  });
