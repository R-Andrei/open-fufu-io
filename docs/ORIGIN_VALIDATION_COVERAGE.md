...
- Controller entity `get`/list surfaces, operations/contacts/events, entity-addressed mechanics/quotes, player/controller debug, and Official AI must all consume the same visibility result. A remembered hidden ID must not become an existence/state oracle, and blind cell-targeted legality/quotes must not leak concealed contents.
