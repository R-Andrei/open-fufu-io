...
Whenever a required integration seam crosses authoritative subsystem ownership because an occurrence owned by one domain causes a consequence owned by another, validation must prove the producer supplies the canonical simulation-domain fact and the consumer applies its consequence through explicit deterministic phase ordering under [`SIMULATION_EVENTS.md`](./SIMULATION_EVENTS.md). A trait-specific callback, listener, private result-delivery channel, or direct mutation of another subsystem is not an equivalent substitute. This requirement does not turn current-state reads or an owner-local atomic transaction into an event merely because an Origin participates in that mechanic.
...
effective transaction / profile
    ↓
authoritative producer resolution / producer-owned commit
    ↓
canonical simulation-domain fact when another subsystem owns a consequence
    ↓
explicitly ordered consumer-owned consequence
```
...
The event step above exists only when the authoritative occurrence crosses an ownership boundary. A current-state dependency remains an ordinary authoritative read, and state that belongs to one atomic owning transaction remains directly committed by that owner. Validation must prove the actual boundary established by [`SIMULATION_EVENTS.md`](./SIMULATION_EVENTS.md), not mechanically maximize the number of event objects.
...
- **P02 — The Era of Humans** — Direct owners: the replacement profile in `ORIGIN_TRAIT_CATALOGUE.md` over the ordinary growth model in `OPEN_FUFU_DESIGN.md`, composed through `RULE_COMPOSITION.md`. Validate every authored profile anchor plus boundary-adjacent interpolation, exact-rational `TotalPopulation / PopulationCapacity` utilization, exact `3/10` and `7/10` breakpoint handling, exact `4/3` remapping/interpolation through the owning-domain materialization boundary, and preservation of independent growth inputs. Explicit interaction: N01; include Capacity states affected by P48 as ordinary upstream state. The normal controller self projection must report truthful authoritative `utilization` and `growthPerSecond` values without exposing a separate raw utilization-multiplier calculator; Official AI consumes the same lawful projection boundary.
...
- **P27 — Only My Railgun** — Direct owner: SAM/strategic-weapon behavior in `NAVAL_AND_STRATEGIC_WEAPONS.md`. Validate anti-ship target legality, damage/cadence, charge arbitration, physical-range qualification, deterministic ship ordering, and strategic-projectile priority exactly as that owner defines them. Explicit interactions: P11, P40, P32, P28. Negative assertions: no Trade-Ship target class, no independent P27 charge/cooldown pool, and no manual controller SAM-fire action.
- **P28 — Blood Devil** — Direct seam: canonical Transport-destruction simulation fact -> game-wide Population accounting. Validate exactly-once frozen carried-Population transfer using canonical destruction attribution and recipient-accounting rules, including uncredited/same-side/non-destruction exclusions and over-Capacity receipt behavior. Explicit interactions: P32, P27, N13; strategic-destruction qualification follows the focused attribution contract.
...
- **P31 — Heart-Under-Blade** — Direct seam: Port two-tier repair service -> Warship repair/operation. Validate the Warship-specific effective broad and fast repair-field/rate projections through the shared two-tier lifecycle, deterministic provider assignment/queueing and non-stacking, health clamping, continued ordinary Warship operation while repairing, and the coexistence of that operational capability with mandatory fast-service parking: an actively fast-serviced Warship remains stationary, while movement exits fast service and re-enters the shared resolver. Explicit interactions: P29, P22/P23; negative assertions that P31 does not change fast-service capacity and that P32 Transports receive only ordinary eligible repair.
...
- **P33 — Misaka Network** — Direct seam: canonical Train-station simulation fact -> Population accounting for the City owner. Validate event ownership/current City level/Capacity clamp independently of the FFY amount. Explicit interactions: P07, P41, P08, P14/P24/N11, P34.
...
- **P35 — It's a Matter of Visualization** — Direct owners: deliberate abandonment in `OPEN_FUFU_DESIGN.md`, persistent-structure occupancy in `TERRAIN_AND_STRUCTURES.md`, and the Origin Fallout transform. Validate whole-command rejection for any structure-occupied selection across completed/damaged/fresh/upgrading structures; validate successful ordinary neutralization before Fallout and assert no capture, nuclear-casualty, structure-capture, or Population-casualty artifact. Explicit interactions: P16, N05, N18, P36, P44.
- **P36 — Half-Priced Bento** — Direct owners: the Origin cost transform plus game-wide Population neutral-settlement accounting. Validate faction-persistent half residual through serial/concurrent settlements, save/load/replay, operation splitting/recreation, ascending-cell deterministic aggregation, aggregate surviving neutral-expansion debit eligibility, deterministic projection of each successful whole debit back onto concrete commitments without operation-ID ordering, and no cross-pool fallback. Selector-key coverage must prove canonical `CELLS` sort/deduplication, recursive `UNION`/`INTERSECTION` flatten/sort/deduplication, ordered `DIFFERENCE`, stable scalar serialization, and fixed-field serialization for both `STRUCTURE_FIELD` and `STRUCTURE_FIELD_INSTANCE`; byte-identical selector-key ties may use only the projection-only stable directive key and must not alter faction-level results. Assert that legal operation partition/recreation or directive-key changes cannot change ownership results, faction Total/Available/aggregate committed Population, residual state, or aggregate same-tick neutral-expansion pressure. Explicit interactions: N18 and P48 pre-acquisition classification timing.
...
- **P44 — Nobel Prize** — Direct seam: canonical successful Tank-chassis Population-attack simulation fact -> deterministic territorial neutralization/Fallout. Validate eligible-cell selection from the canonical owner, neutralization rather than capture, shared Fallout semantics, and no structure-capture consequences. Explicit interactions: P43, P16, N05, P35; cross-faction P44 attacker -> P48 defender; negative P47 assertion.
- **P45 — Hidden Leaf Village** — Direct owner: observation/concealment projection. Validate that every holder-owned Forest cell qualifies with no adjacency, frontage, narrow-strip, ownership-hole, or map-edge exception; terrain/ownership remain public; viewer-owned state remains visible; regional concealment filters otherwise non-public opposing/third-party state; ownership/terrain changes re-evaluate immediately; ordinary observation cannot bypass concealment; and a qualifying hostile manifestation exposes only its source through the canonical direct-reveal contract. Explicit interaction: P49 union; combat and all controller/AI-derived surfaces receive no hidden-state bypass.
...
- **P47 — This Is Poison** — Direct owners: the Origin trigger plus game-wide successful-capture Population accounting. Validate the extra capturing-faction casualty after ordinary capture casualties, faction-level aggregation of all same-tick requests before allocation, aggregate surviving Population across every qualifying winning commitment as the first source class, Available Population second, deterministic projection from canonical capture facts without operation-ID ordering, and clamp when both source classes are exhausted. Assert that legal splitting/recreation of otherwise equivalent qualifying attacks cannot change the faction's realized P47 loss, including a case where one qualifying winning commitment has zero surviving Population and another has enough surviving Population to exhaust the full aggregate `winningLoss`. Explicit interactions: P38 and cross-faction N18 attacker -> P47 defender; negative assertions that P35 relinquishment and P44 neutralization do not trigger it and that non-qualifying offensive/counter-response/Transport Population is untouched. Controller and Official-AI reasoning must not depend on a private broad-mechanics oracle for this debit order; the authoritative resolver remains the arithmetic owner and both reason from the same lawful information boundary.
...
- **P49 — Laughing Man** — Direct owner: Observation-Post effective observation profile. Validate `REVEAL -> ENEMY_BLACKOUT` replacement rather than addition, reuse of the same authoritative effective Observation field, public Post/field existence and extent, activation/upgrade/ownership-transfer lifecycle, viewer-self visibility, regional suppression of otherwise non-public opposing/third-party state, overlap union without strength, remote-observation suppression, and direct-reveal precedence through the canonical projection. Explicit interactions: P45, P46, N17.
...
- **N13 — Beach Episode Gone Wrong** — Direct owner: amphibious landing Population resolution. Validate the canonical landing transition and whole-number rounding, including odd-payload and zero-survivor cases, and prove it never fires on embark/destruction/abort/return paths. Explicit interactions: P37, P32, P12, N15, P28.
...
- P36 + P48 — qualification uses the neutral target's pre-acquisition effective classification, while later P48 ownership classification updates Capacity normally.
...
- P38 defender + P47 defender trait — P38's defender survival changes no P47 capturing-side debit semantics.
- P47 operation partition — identical qualifying capture facts and aggregate surviving qualifying-winning Population produce the same faction-level P47 loss regardless of legal operation split/recreation; operation identity may not change the source-class totals.
- P44 attacker -> P48 defender — target eligibility uses the defender's faction-effective population-bearing classification.
- P35/P44 -> P47 holder — abandonment/neutralization are negative trigger cases because neither is a hostile successful capture.
...
- P27 + P40 — anti-ship SAM behavior uses the same effective range/charge/recharge state as strategic-projectile interception, including same-tick projectile-first charge arbitration.
- P27 + P32 — the transformed health-bearing Transport consumes the ordinary P27 anti-ship damage path rather than a baseline fragile-Transport shortcut.
- P27 + P28 — a P27 Transport kill must produce the same canonical credited Transport-destruction result consumed by P28; no P27-specific bounty path exists.
- P28 + N13 — destruction-before-landing and landing-completion are mutually exclusive terminal payload paths; the same carried Population must never be both stolen and processed as an N13 landing.
...
- P45 + P49 — concealment predicates compose as continued concealment/union through one lawful requester-relative visibility projection, not numeric stacking.
- P45/P49 hostile-manifestation exceptions consume the canonical full-source direct-reveal precedence/lifetime/expiry/refresh contract; only an actually resolved hostile effect from an identifiable source qualifies, reveal follows source identity through movement, and no neighboring concealed state is surfaced.
- Ordinary Observation and P49 blackout must consume the same authoritative Observation-field geometry; P49 replaces rather than supplements the Post's reveal effect.
- Controller entity `get`/list surfaces, known-ref operations/events, focused public `check*` results, player/controller debug, and Official AI must all consume the same visibility result. A remembered hidden ref must not become an existence/state oracle, and blind cell-targeted checks must not leak concealed contents. There is no separate normal Contacts or broad mechanics query surface that may bypass this boundary.
- Deterministic save/load/replay and regeneration must reproduce direct-reveal recipient/source identity, canonical expiry/refresh transitions, Observation/blackout field membership, P45 ownership/terrain qualification, and the same projected visibility on the same simulation tick.
...
- Every required integration seam that crosses subsystem ownership must identify the producer-owned occurrence/fact, the explicitly ordered consumer, and the owning consequence, and must prove that no trait-private callback/listener/result-delivery channel substitutes for the canonical `SIMULATION_EVENTS.md` boundary. If the focused owners have not yet established the required factual occurrence/context, that seam is not certifiable merely by inventing one in validation metadata.
