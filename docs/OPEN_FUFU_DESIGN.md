...
Current observation contains no engine-created `lastKnown` substitute for a subject that has reconcealed. Controllers may retain lawful historical knowledge in their own memory. A previously acquired controller-facing object reference is not a visibility or liveness capability: no public or derived surface may disclose current existence, location, state, or a distinguishable failure solely because an otherwise hidden subject still exists. Blind cell-addressed actions remain legal where their owning subsystem permits them, but validation or prospective information must not disclose concealed contents of the target cell.
